...
int SendbyPost(const char *sendtype,const char *path,const char *IPSTR,const char *PORT,unsigned const char *str,int str_len,unsigned char *res)
{
	...
	{
        ERROR_PRINT("Create socket failed!\n");
        return ret;
	};
//	printf("socket fd =[%d]\n",sockfd);
	...
	INFO_PRINT("Connect success !\n");

	strlen_buf=jointPostPkg(sendtype,path,IPSTR,PORT,str,str_len,strbuf);
	//printf("strlen_buf=[%d],strbuf=[%s]\n",strlen_buf,strbuf);
	
	ret = write(sockfd,strbuf,strlen_buf);
	if (ret < 0) {
        ERROR_PRINT("Send error ! error code =[%d] , error message =[%s]\n",errno, strerror(errno));
        return ret;
	}
	else
	{
        DEBUG_PRINT("Send success for [%d] bytes !\n\n", ret);
	}
	//
	...
	while(1)
	{
		...
		if (h == -1) {
			close(sockfd);
			ERROR_PRINT("Socket error! Read failed !\n");
			return -1;
		};
		if ( FD_ISSET(sockfd, &t_set1) )
		{
			...
			if (i==0){
				close(sockfd);
				ERROR_PRINT("Cannot connect to server !\n");
				return -1;
			}
			//printf("%s\n", res);
			break;
...
int splitRecvPkg(unsigned char *instr,CicvserverRespond_t *serverRespond)
{
	...
	//printf("outstr[%d]=[%s]\n",i,&outstr[i]);
	respondSave(outstr,flag,serverRespond);
	i++;
	while(1)
	{
		...
		if(*(p+1) == '\n')
		{
			...
			if(*(p+2) == '\r')
			{
				flag=1;
				p=p+4;
				respondSave(p,flag,serverRespond);
				break;
			}
			p = p+2;
		}
